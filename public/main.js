...
const viewportToLocal = (width, height) => arr => [-width / 2 + arr[0], -(-height / 2 + arr[1])];
...
function permute$1(a, p) {
  var r = [];
  for (var i = 0; i < a.length; ++i) r.push(a[p[i]]);
  for (i = 0; i < a.length; ++i) a[i] = r[i];
...
var underscore_2 = underscore$1.debounce;
...
var mediator = { subscribe, publish };
...
window.addEventListener("resize", underscore_2(handleResize, 250));
...
const getDistance = {
  'euclidean': vectorLength$2,
  'manhattan': manhattanLength$2,
...
class Dropdown extends Component {
  render({ options: options$$1, change }) {
    return h(
      'select',
      { onChange: e => change(e.target.value), 'class': 'options' },
      options$$1.map(d => {
        return h(
          'option',
          {
            ...
            value: d.index,
            selected: d.selected, 'class': 'option' },
          d.sentence ? d.sentence : d.index
        );
      })
    );
  }
    ...
    return h(
      ...
      options$$1.map(d => {
        return h(
          ...
          {
            value: d.id,
            selected: d.active, 'class': 'option' },
          d.label
        );
      })
    );
...
let radiusScale = linear().domain([-0.1, 0.1]).range([0, spokeLength]);
let radialLine$$1 = lineRadial$1();
let encodingsDict = {};
let wassersteinPairwise$1;
let sentenceDict$1 = {};

const getDistance$1 = {
  'euclidean': vectorLength$4,
  'manhattan': manhattanLength$4,
  'fractional': fractional$2(0.5),
  'wasserstein': function (vec, startIndex, endIndex, dim) {
    return wassersteinPairwise$1[dim][startIndex][endIndex];
  }
};

class Dropdown$2 extends Component {
  render({ options: options$$1, change }) {
    return h(
      'select',
      { onChange: e => change(e.target.value), 'class': 'options' },
...
let radiusScale$1 = linear().domain([-0.1, 0.1]).range([0, spokeLength$1]);
let radialLine$1 = lineRadial$1();
let encodingsDict$1 = {};

const getDistance$2 = {
  'euclidean': vectorLength$5,
  'manhattan': manhattanLength$5,
  'fractional': fractional$3(0.5)
};

class Dropdown$3 extends Component {
  render({ options: options$$1, change }) {
    return h(
      'select',
      { onChange: e => change(e.target.value), 'class': 'options' },
...
class Dropdown$4 extends Component {
  render({ options: options$$1, change }) {
    return h(
      'select',
      { onChange: e => change(e.target.value), 'class': 'options' },
...
let graphHeight$1 = 100;
let graphXIncrement$1 = 0;

class Dropdown$5 extends Component {
  render({ options: options$$1, change }) {
    return h(
      'select',
      { onChange: e => change(e.target.value), 'class': 'options' },
...
class EmbeddingSpiral extends Component {
  ...
  componentDidUpdate() {
    ...
    graphXIncrement$1 = 400 / sparklinePoints[0].length;

    distances.forEach((dist, di) => {
      let max = Math.max(...sparklinePoints[di]);
      select(document.querySelector(`#svg_${dist}`)).select("path").attr("d", line().x((d, i) => i * graphXIncrement$1).y(d => (1 - d / max) * graphHeight$1)(sparklinePoints[di]));
  ...
  render({}, { sentences, models, manipulations, distances, hoverIndex }) {
    ...
    if (activeSentenceIndex > -1) {
      sentencesDOM = activeManipulation.sentences[activeSentenceIndex].manipulations.map((d, i) => {
        return h(
          'div',
          { 'data-active': i === hoverIndex, onMouseOver: () => {
    ...
    return h(
      ...
      h(
        'div',
        { 'class': 'sentences-wrapper' },
        sentencesDOM
      ),
      h(
        'div',
        { 'class': 'controls' },
        h(Dropdown$5, { change: id => this.changeDropdown(id, 'sentences'), options: sentences }),
        h(Dropdown$5, { change: id => this.changeDropdown(id, 'models'), options: models }),
        h(Dropdown$5, { change: id => this.changeDropdown(id, 'manipulations'), options: manipulations }),
        h('br', null),
        h(
          'div',
          { 'class': 'distances-wrapper' },
          distances.map(d => {
            return h(
              'div',
              ...
              h(
                'svg',
                { id: `svg_${d}` },
                h('text', { y: '12', id: `text_min_${d}` }),
                h('text', { y: '24', id: `text_max_${d}` }),
                h('text', { y: '36', id: `text_curr_${d}` }),
                h('line', { x1: hoverIndex * graphXIncrement$1, x2: hoverIndex * graphXIncrement$1, y1: '0', y2: graphHeight$1 }),
                h('path', null)
              )
            );
          })
...
class App extends Component {
  render({ data }) {
    ...
    let main = h(EmbeddingSpiral, null);
