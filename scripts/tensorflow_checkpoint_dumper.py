...
class TensorflowCheckpointDumper(CheckpointDumper):
  ...
  def build_and_dump_vars(self):
    ...
    print("build and dump vars")
    print(var_to_shape_map)

    for (var_name, var_shape) in iteritems(var_to_shape_map):
      print(var_name)
      if self.should_ignore(var_name) or var_name == 'global_step' or 'logits/weights/Adam' in var_name or 'logits/weights/Adam_1' in var_name or 'word_embedding' in var_name:
        print('Ignoring')
        continue
